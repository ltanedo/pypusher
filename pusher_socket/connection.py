...
class Connection(Thread):
    def __init__(self, event_handler, url, reconnect_handler=None, log_level=None,
                 daemon=True, reconnect_interval=10, socket_kwargs=None, pusher_obj=None, **thread_kwargs):
    ...
    def add(self, channel, event, func):
    ...
    def _connect(self):
        ...
        while self.needs_reconnect and not self.disconnect_called:
            self.logger.info("Attempting connection in %s s."
    ...
    def _on_message(self, *args):
        ...
        # pusher:pong
        if "pusher_internal:subscription_succeeded" not in message\
            and "pusher:connection_established" not in message\
                and "pusher:pong" not in message:
                    self.logger.info("[Message] \n%s\n\n" % json.dumps(json.loads(message), indent=2) )

        if "pusher:connection_established" in message:
            ...
            for record in self.holdings:
                ...
                if record['channel'] not in unique_channels:
                    unique_channels[record['channel']] = self.pusher_obj.subscribe(record['channel'])
                
                channel = unique_channels[record['channel']]
                channel.bind(record['event'], record['func'])
